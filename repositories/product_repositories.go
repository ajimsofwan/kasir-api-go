...
func (repo *ProductRepository) GetAll() ([]models.Product, error) {
	query := " SELECT  p.id, p.name, p.price, p.stock, p.category_id, COALESCE(c.name, '') as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id ORDER BY p.id ASC"
...
func (repo *ProductRepository) GetByID(id int) (*models.Product, error) {
	query := "SELECT  p.id, p.name, p.price, p.stock, p.category_id, COALESCE(c.name, 'Tanpa Kategori') as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = $1"
